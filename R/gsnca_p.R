...
gsnca_p <- function(object,group,perm.list,cor.method='pearson',max.skip=50,min.sd=0.001) {
    objt1 <- t(object[,group==1])
    objt2 <- t(object[,group==2])
    D_obs <- gsnca_stat(objt1,objt2,cor.method=cor.method)
    nv <- ncol(object)
    ...
    for (itr in seq_len(nperm)) {
			...
			D_perm[itr] <- gsnca_stat(objt1,objt2,cor.method=cor.method)
    }
